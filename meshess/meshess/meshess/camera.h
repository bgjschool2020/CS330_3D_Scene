...
public:
	...
	glm::mat4 GetProjectionMatrix(float width, float height, float fieldOfView) const {
		auto aspectRatio = width / height;
		if (IsPerspective) {
			return glm::perspective(glm::radians(fieldOfView), aspectRatio, nearClip, farClip);
		}

		return glm::ortho(-aspectRatio * OrthoZoom, aspectRatio * OrthoZoom, OrthoBottom * OrthoZoom, OrthoTop * OrthoZoom, nearClip, farClip);
	}
	...
	void ProcessMouseScroll(float yoffset)
	{
		if (IsPerspective == true) {
			...
			if (MovementSpeed < 1.f)
				MovementSpeed = 1.f;
			if (MovementSpeed > 7.f)
				MovementSpeed = 7.f;

			std::cout << "Camera Movement Speed is now set to " << MovementSpeed << std::endl;
		}
		else {
			OrthoZoom -= yoffset / 2;
			...
			std::cout << "Orthographic Zoom now set to " << OrthoZoom << std::endl;
		}
	}
